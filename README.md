...
1. **❓ Do I already have Orange Pi?**
   - ✅ YES, it's set up → Jump to [Linux Installation](#for-linux--orange-pi--raspberry-pi)
...
2. **❓ Can I do this with my Windows PC instead?**
   - ✅ YES! Follow [Windows Installation](#for-windows)
...
### Quick Reference
- [⚡ Quick Reference Card](#quick-reference-card) ⭐ **Bookmark this!**
...
### Optional Features
- [🌐 Optional: Remote Access with Tailscale](#-optional-remote-access-with-tailscale) *(Choose later if needed)*
- [↩️ Migrate Existing Installation to Tailscale](#migrate-existing-installation-to-tailscale)
...
- [🖼️ Dashboard Setup](#lovelace-dashboard)
  - [Step 1: Verify Card Files](#step-1-verify-card-files)
  - [Step 2: Add Card as a Resource](#step-2-add-card-as-a-resource)
  - [Step 3: Create Your Dashboard](#step-3-create-your-dashboard)
  - [📸 Visual Guides](#step-2-add-card-as-a-resource) *(includes img.png, img_1.png)*
...
- [🚀 Tailscale VPN Setup](#-tailscale-vpn-setup)
  ...
  - [🐧 Linux Installation (Orange Pi, Raspberry Pi, Ubuntu)](#-linux-installation-orange-pi-raspberry-pi-ubuntu)
  - [🪟 Windows Installation](#-windows-installation)
  - [🔧 Configuration Details](#-configuration-details)
  - [🛡️ Security Considerations](#-security-considerations)
  - [📱 Mobile Access](#-mobile-access)
  - [🔧 Troubleshooting](#-troubleshooting)
  - [🔄 Integration with Setup Scripts](#-integration-with-setup-scripts)
...
### <a id="-optional-remote-access-with-tailscale"></a>🌐 Optional: Remote Access with Tailscale

**Do you need to access your solar system from outside your home?**
...
- You want to monitor from work/travel
- You need family to view the dashboard
- You want secure internet-free access

**→ [Follow Tailscale Setup](#-tailscale-vpn-setup)**

Takes ~5 minutes extra to set up.

</td>
<td width="50%">

#### ❌ NO - Local Network Only
- You only access at home on WiFi
- You don't need remote monitoring
- You prefer simpler setup

**→ Skip Tailscale section entirely**

Just use regular `docker-compose.yml`

</td>
</tr>
</table>

**Can I Add Tailscale Later?**
✅ **YES!** You can:
1. Install without Tailscale first
2. Add Tailscale anytime later without losing configuration
3. See "Migrate Existing Installation to Tailscale" section below for details

---

### <a id="migrate-existing-installation-to-tailscale"></a>Migrate Existing Installation to Tailscale

**Already installed without Tailscale? Easy to add:**

```bash
# 1. Backup your configuration (safe)
cp -r /root/solarman/homeassistant_config /root/solarman/homeassistant_config.backup

# 2. Copy Tailscale compose file
cp docker-compose.tailscale.yml docker-compose.yml

# 3. Create .env file with your Tailscale auth key
# Get key from: https://login.tailscale.com/admin/settings/keys
cp .env.example .env
# Edit .env and add your auth key

# 4. Restart with Tailscale
docker compose down
docker compose up -d

# ✅ Done! Your configuration is preserved.
```

Your Home Assistant data is safe - it's all in the volumes!
...
## <a id="quick-reference-card"></a>⚡ Quick Reference Card

**Essential commands for users - bookmark this!**
...
# Check Tailscale VPN status (if using Tailscale)
tailscale status
...
# Full restart (stop everything and start fresh)
...
```bash
# Home Assistant config directory (Linux)
/root/solarman/homeassistant_config

# Home Assistant config directory (Windows)
C:\solarman\homeassistant_config

# Environment variables file (for Tailscale)
.env
...
nano /root/solarman/homeassistant_config/configuration.yaml
...
| Problem | Solution |
|---------|----------|
| **Dashboard won't load** | Restart: `docker restart homeassistant` |
| **Sensors show "Unavailable"** | Check inverter IP in integration settings |
| **"Port 8123 already in use"** | Change port in docker-compose.yml |
| **Can't find Tailscale IP** | Run: `tailscale ip -4` |
| **Docker daemon not running** | Start Docker Desktop (Windows) or run: `sudo systemctl start docker` (Linux) |
| **Permission denied errors** | Run with sudo: `sudo docker ...` |
| **Configuration won't validate** | Check YAML syntax: `docker exec homeassistant ha core check` |

### Useful Paths & URLs

```
Local Access (on your network):
  • Home Assistant UI: http://localhost:8123
  • From another device: http://192.168.x.x:8123 (replace x.x with your IP)

Remote Access (with Tailscale):
  • Home Assistant UI: http://[tailscale-ip]:8123 (e.g., http://100.64.1.3:8123)

Configuration Files:
  • Linux: /root/solarman/homeassistant_config/
  • Windows: C:\solarman\homeassistant_config\
  
Logs Directory:
  • Linux: /root/solarman/homeassistant_config/.homeassistant.log
  • Windows: C:\solarman\homeassistant_config\.homeassistant.log
```
...
# Check memory usage
docker exec homeassistant free -h

# View Home Assistant database size
...
cp -r /root/solarman/homeassistant_config /root/solarman/homeassistant_config.backup.$(date +%Y%m%d)

# Restore from backup
cp -r /root/solarman/homeassistant_config.backup.20260301 /root/solarman/homeassistant_config

# Create timestamped backup
docker compose down
cp -r /root/solarman/homeassistant_config /root/solarman/backups/config_$(date +%Y%m%d_%H%M%S)
docker compose up -d
```
...
netstat -tuln | grep 8123  # (nothing = available)

# Check if Docker daemon is running
sudo systemctl status docker  # (Linux) or check Docker Desktop (Windows)
```
...
- Sunsynk Power Flow dashboard card
- Tailscale VPN (optional, automated)

⏱️ **Installation time:** 5-10 minutes (first run) or 1-2 minutes (with Docker already installed)

🌐 **Access after installation:** `http://localhost:8123`

> **Next:** Jump to [Home Assistant Setup](#home-assistant-setup) to configure your inverter
...
### <a id="step-1-verify-card-files"></a>Step 1: Verify Card Files Are Installed
...
### <a id="step-2-add-card-as-a-resource"></a>Step 2: Add Card as a Resource
...
4. Click **"+ ADD RESOURCE"** button (bottom right)
   ...
   ![Add Resource Button](images/img.png)
...
6. Click **"CREATE"**
   ...
   ![Resource Configuration](images/img_1.png)

7. **Refresh your browser** (Press `Ctrl+Shift+R` or `Cmd+Shift+R`)

---

### <a id="step-3-create-your-dashboard"></a>Step 3: Create Your Dashboard
...
**Never lose access to your solar monitoring system!** Tailscale provides secure, zero-configuration remote access to your Home Assistant installation.

**Benefits:**
- 🔒 **Military-grade encryption** - All traffic is encrypted end-to-end
- 🌍 **Works anywhere** - Access from anywhere in the world without opening firewall ports
- 📱 **Mobile access** - View your solar system on your phone while away from home
- 🛡️ **Zero-trust security** - Only authorized devices can access your system
- ⚡ **No technical setup** - Automatic NAT traversal and peer-to-peer connections

**What you'll need:**
- Tailscale account (free for personal use)
- Tailscale app installed on your devices
- Your Home Assistant running with Tailscale integration

**Setup time:** 5 minutes | **Difficulty:** Beginner-friendly | **Cost:** Free

**How it works:**
1. Tailscale creates a secure VPN between your devices
2. Your Home Assistant gets a private IP address on the Tailscale network
3. Access your dashboard from anywhere using the Tailscale app
4. All connections are encrypted and authenticated

**Perfect for:**
- Monitoring your system while on vacation
- Checking energy usage from work
- Sharing access with family members
- Remote troubleshooting and maintenance

**Quick Start:**
1. Install Tailscale on your Orange Pi/PC during setup (automated)
2. Install Tailscale app on your phone/computer
3. Sign in with the same account on all devices
4. Access Home Assistant at `http://[tailscale-ip]:8123`
...
**Secure remote access to your Home Assistant installation with Tailscale VPN**

Tailscale provides a secure, zero-configuration VPN solution that allows you to access your Home Assistant installation from anywhere in the world without opening firewall ports or configuring complex networking.
...
#### 🐧 Linux Installation (Orange Pi, Raspberry Pi, Ubuntu)
...
Use the provided `docker-compose.tailscale.yml` file:

```bash
# Copy the Tailscale configuration
cp docker-compose.tailscale.yml docker-compose.yml

# Start Home Assistant with Tailscale
docker compose up -d
```

**Step 4: Access Home Assistant Remotely**

Access your Home Assistant from anywhere using your Tailscale IP:

```
http://100.64.123.45:8123
```
...
2. Note your IP address (e.g., 100.64.123.45)

**Step 3: Configure Docker Compose**

Use the provided `docker-compose.tailscale.windows.yml` file:

```powershell
# Copy the Windows Tailscale configuration
Copy-Item docker-compose.tailscale.windows.yml docker-compose.yml
...
TS_EXTRA_ARGS=--advertise-tags=tag:homeassistant
```
...
2. Click "Create key"
3. Select "Reusable" and "Auth key"
4. Copy the generated key
5. Add it to your `.env` file

**Docker Compose Configuration**

The Tailscale configuration uses a sidecar pattern where Home Assistant runs in the same network namespace as Tailscale:
...
services:
  ...
  tailscale:
    ...
    environment:
      - TS_AUTHKEY=${TS_AUTHKEY:?Please set TS_AUTHKEY environment variable}
...
**Access Control**
- Only devices signed into your Tailscale account can access
- Use Tailscale ACLs for fine-grained access control
- Consider using separate Tailscale accounts for different users

**Best Practices**
- Use strong Tailscale account passwords
- Enable 2FA on your Tailscale account
- Regularly review connected devices
- Use descriptive hostnames for your devices
...
**Access Home Assistant on Mobile**

1. Open Tailscale app and ensure you're connected
2. Open your browser
3. Navigate to `http://[your-tailscale-ip]:8123`
4. Use your Home Assistant credentials to log in
...
1. **"Device not found"**
   ...
   - Check that both devices are signed into the same Tailscale account
   - Verify internet connectivity

2. **"Connection refused"**
   - Check that Home Assistant is running
   - Verify the Tailscale IP address is correct
   - Ensure port 8123 is accessible

3. **"Authentication required"**
   - Sign in to Tailscale on your device
   - Check that your Tailscale account is active
   - Verify your auth key is correct

4. **"Slow connection"**
   - Check your internet speed
   - Try connecting to a different Tailscale exit node
   - Verify your Home Assistant server performance

5. **"Can't find Tailscale IP"**
   - Run `tailscale ip -4` to get your IP
   - Check Tailscale app for device status
   - Ensure Tailscale service is running

6. **"Docker container won't start"**
   - Check that TUN device is available
   - Verify Docker permissions
   - Check Tailscale auth key format
...
The setup scripts now include automatic Tailscale installation:

**Linux Setup Script**
- Automatically installs Tailscale during setup using official installer
- Enables and starts `tailscaled` service automatically
- Verifies Tailscale service is running after installation
- Provides clear instructions in output for connecting to Tailscale:
  ```
  🌐 Remote Access with Tailscale:
     1. Connect to Tailscale: sudo tailscale up
     2. Visit the URL shown to authenticate
     3. Get your Tailscale IP: tailscale ip -4
     4. Access Home Assistant remotely: http://[tailscale-ip]:8123
     5. Install Tailscale app on mobile for remote access
  ```

**Windows Setup Script**
- Downloads and installs Tailscale automatically
- Starts Tailscale Windows service automatically
- Verifies Tailscale service is running after installation
- Provides clear instructions in output for connecting to Tailscale:
  ```
  🌐 Remote Access with Tailscale:
     1. Connect to Tailscale: Start Tailscale app and sign in
     2. Get your Tailscale IP: tailscale ip -4
     3. Access Home Assistant remotely: http://[tailscale-ip]:8123
     4. Install Tailscale app on mobile for remote access
  ```

**What happens automatically:**
- ✅ Tailscale is installed during the main setup script execution
- ✅ Tailscale service is enabled to start on boot
- ✅ Tailscale service is started automatically
- ✅ You are guided through the authentication process after installation
- ✅ Scripts verify successful installation and service status
...
1. **Update Tailscale**: Keep Tailscale updated for security
2. **Review devices**: Regularly check connected devices
3. **Monitor usage**: Watch for unusual network activity
4. **Backup configs**: Keep backup of your configurations

**Performance Optimization**

- Use exit nodes for better routing
- Configure split tunneling if needed
- Monitor bandwidth usage
- Consider upgrading to paid plan for advanced features

This Tailscale integration provides enterprise-grade security for accessing your solar monitoring system from anywhere in the world, making it perfect for remote monitoring and management of your solar installation.
...
```
srne-solarman/
├── .env.example                                # Environment variables template (Tailscale auth)
├── .gitignore                                  # Git ignore rules
├── CONTRIBUTING.md                             # Contribution guidelines
├── README.md                                   # This documentation
├── LICENSE                                     # MIT License
├── templates/                                  # Configuration files
│   ├── configuration.yaml                      # Home Assistant base configuration
│   └── srne_hesp.yaml                          # SRNE inverter sensor definitions
├── docker/                                     # Docker Compose files
│   ├── docker-compose.yml                      # Linux Docker configuration
│   ├── docker-compose.windows.yml              # Windows Docker configuration
│   ├── docker-compose.tailscale.yml            # Linux Docker + Tailscale VPN
│   └── docker-compose.tailscale.windows.yml    # Windows Docker + Tailscale VPN
├── images/                                     # Documentation images
│   ├── img.png                                 # Dashboard resource button screenshot
│   └── img_1.png                               # Dashboard configuration screenshot
...
**Directory Details:**
- `docker/` - Docker Compose files for different deployment scenarios
- `images/` - Screenshots and documentation images
- `scripts/` - Automated setup scripts for Linux and Windows
