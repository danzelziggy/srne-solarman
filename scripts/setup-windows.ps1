...
$composeFile = Join-Path $repoRoot 'docker\docker-compose.windows.yml'
...
function Install-Prerequisites {
    ...
    Write-Host "🌐 Installing Tailscale for remote access..." -ForegroundColor Cyan
    Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Cyan
    if (-not (Get-Command tailscale -ErrorAction SilentlyContinue)) {
        Write-Step "Downloading and installing Tailscale..."
        $tailscaleUrl = "https://pkgs.tailscale.com/stable/tailscale-setup-1.80.0.exe"
        $tailscaleInstaller = Join-Path $env:TEMP "TailscaleInstaller.exe"
        
        try {
            Invoke-WebRequest -Uri $tailscaleUrl -OutFile $tailscaleInstaller -UseBasicParsing
            Write-Step "Installing Tailscale (this may take a moment)..."
            ...
            if (Get-Service -Name "Tailscale" -ErrorAction SilentlyContinue | Where-Object {$_.Status -eq 'Running'}) {
                Write-Success "Tailscale service is running"
            } else {
                Write-Warning-Custom "Tailscale service may not be running properly"
                Write-Warning-Custom "You may need to start it manually from Services"
            }
        } catch {
            Write-Warning-Custom "Failed to install Tailscale: $_"
            Write-Warning-Custom "You can install Tailscale manually from: https://tailscale.com/download"
        }
    } else {
        ...
        if (Get-Service -Name "Tailscale" -ErrorAction SilentlyContinue | Where-Object {$_.Status -eq 'Running'}) {
            ...
            Write-Warning-Custom "Tailscale service not running, attempting to start..."
            try {
                Start-Service -Name "Tailscale"
                Write-Success "Tailscale service started"
            } catch {
                Write-Warning-Custom "Failed to start Tailscale service: $_"
            }
        }
    }
}
...
"@
    ...
    try {
        ...
        if ($containerExists) {
            ...
            docker compose -f docker\docker-compose.windows.yml up -d | Out-Null
    ...
    Write-Host ""
    Write-Host "🌐 Remote Access with Tailscale:" -ForegroundColor Cyan
    Write-Host "   1. Connect to Tailscale: Start Tailscale app and sign in"
    Write-Host "   2. Get your Tailscale IP: tailscale ip -4"
    Write-Host "   3. Access Home Assistant remotely: http://[tailscale-ip]:8123"
    Write-Host "   4. Install Tailscale app on mobile for remote access"
    Write-Host ""
